...
class ClientController extends Controller
{
    ...
    public function fix_repay(){
        echo 1;
        //$repay= LoanTransaction::where('loan_id','0')->get();
       //// print_r($repay);
    }
...
// PAYMENT INVALID & INVESTIGATE MANUALY!
  ...
  return 0;
   ...
   try {
            $loan = Loan::find($account);
        if(!empty($loan)){
            $loan_transaction = new LoanTransaction();
            $loan_transaction->branch_id = $loan->branch_id;
            $loan_transaction->loan_id = $account;
            $loan_transaction->borrower_id = $loan->borrower_id;
            $loan_transaction->transaction_type = "repayment";
      ...
      $borrower=Borrower::where('id_no',$account)->first();
        if(!empty($borrower)) {

           // $income=OtherIncome::where('member_id',$borrower->id)->first();
            ...
            $borrower=Borrower::where('unique_number',$account)->first();
             if(!empty($borrower)) {
               ...
               $other_income->notes = "Mpesa:" . $phone;
               $other_income->date = date("Y-m-d");
               $other_income->member_id=$borrower->id;
               $date = explode('-', date("Y-m-d"));
    ...
    public function mpesa_c()
    {
        ...
        $borrower=Borrower::where('unique_number',"KC".(sprintf('%03s',$account)))->first();
       if(!empty($borrower)) {
